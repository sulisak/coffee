...
$query = $this->db->query('SELECT
    ...
    (SELECT sum(sd.importproduct_detail_num) FROM purchase_buy_detail as sd WHERE sd.product_id=wpl.product_id  AND sd.owner_id="'.$_SESSION['owner_id'].'" AND sd.importproduct_detail_date BETWEEN "'.$dayfrom.'" AND "'.$dayto.'") as product_numall,
    (SELECT sum(sd.price_per_num * sd.importproduct_detail_num) FROM purchase_buy_detail as sd WHERE sd.product_id=wpl.product_id   AND sd.owner_id="'.$_SESSION['owner_id'].'" AND sd.importproduct_detail_date BETWEEN "'.$dayfrom.'" AND "'.$dayto.'") as product_priceexpenall
...
AND sld.importproduct_detail_date
BETWEEN "'.$dayfrom.'"
AND "'.$dayto.'"
...
$query = $this->db->query('SELECT *, from_unixtime(importproduct_detail_date,"%d-%m-%Y %H:%i:%s") as adddate
FROM purchase_buy_detail
WHERE owner_id="'.$_SESSION['owner_id'].'"
AND importproduct_detail_date
BETWEEN "'.$dayfrom.'"
AND "'.$dayto.'"
AND product_id="'.$data['product_id'].'" ORDER BY importproduct_detail_id ASC');
